...
Args parseArgs(int argc, char **argv) {
    argparse::ArgumentParser program("yolov8-openvino-cpp");
    program.add_description("YOLOv8 OpenVINO detection");

    program.add_argument("--model").help("Path to the YOLOv8 ONNX model.").metavar("MODEL");
    ...
    program.add_argument("--gpu").default_value(false).help("Whether to use GPU");

    program.add_argument("--score-thresh").default_value(0.25f).help("Minimum confidence threshold").scan<'g', float>();
    ...
    std::cout << "🖼️" << LogInfo(" Detect", "model=" + args.modelPath) << " source=" + args.source << " imgsz=["
              << args.imgSize[0] << "," << args.imgSize[1] << "]"
              << " device=" << (args.gpu ? "true" : "false") << " score-tresh=" << args.scoreThresh
              << " iou-thresh=" << args.iouThresh << std::endl;
